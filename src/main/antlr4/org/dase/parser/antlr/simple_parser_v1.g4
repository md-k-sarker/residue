grammar simple_parser_v1;

// to test: antlr4 -o . -lib . -no-listener -no-visitor simple_parser_v2.g4; javac *.java; grun simple_parser_v6 parsecandidatesolution -gui testcase13.png.txt
// ref: https://riptutorial.com/antlr/example/11231/setup-testrig
// omit the header for test purpose
...
* parser rules
*/
parsecandidatesolution  : g=groupedcandidateclasses ( AND groupedcandidateclasses )* #parseCandidateSolution  ;
groupedcandidateclasses : SOME parseobjectpropertyid DOT parsecandidateclasswithprop (OR parsecandidateclasswithprop)* #parseGroupedCandidateClassesWithProp |
                          parsecandidateclass ( AND parsecandidateclass)* #parseGroupedCandidateClassesWithoutProp  ;
// this rule has problem....
parsecandidateclasswithprop :  OPENPAR? parseconjunctivehornclause ( OR parseconjunctivehornclause)* CLOSEPAR? #parseCandidateClassWithProp ;
parsecandidateclass : OPENPAR?  parseconjunctivehornclause ( AND parseconjunctivehornclause)* CLOSEPAR?  #parseCandidateClassWithoutProp  ;
parseconjunctivehornclause : OPENPAR? OPENPAR? parseposclasses CLOSEPAR? ( AND NOT OPENPAR parsenegclasses CLOSEPAR )? CLOSEPAR? #parseConjunctivehornclause ;
parseposclasses  :  parseclassid ( AND parseclassid )* #parsePosClasses ;
parsenegclasses :  parseclassid ( OR parseclassid )* #parseNegClasses ;
parseobjectpropertyid : parseid #parseObjProp;
parseclassid : parseid #parseClass;
parseid : IDD  #parseID ;
